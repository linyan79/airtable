...
namespace AirtableApiClient
{
    public class AirtableBase : IDisposable
    {
        ...
        public Uri BuildUriForListRecords(
            string tableName,
            string offset,
            ...
            string view)
        {
            ...
            if (query.Count > 0)
            {
                ...
                {
                    uriBuilder.Query = uriBuilder.Query.Substring(1) + "&" + Decode(query.ToString());
                }
                else
                {
                    uriBuilder.Query = Decode(query.ToString());
                }
            }
            ...
            //uriBuilder.Query = Decode(uriBuilder.Query);

            return uriBuilder.Uri;
        }

        static string Decode(string tx)
        {
            return tx;
        }
